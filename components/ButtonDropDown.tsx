...
export function ButtonDropDown({nodeId, isOpen, setIsOpen} : {
  ...
  const { duplicateNode, deleteNode } = useNodeActions();
    ...
    <DropdownMenu open={isOpen} onOpenChange={() => setIsOpen(prev => !prev)}>
      <DropdownMenuTrigger asChild>
        <Button
          className="nodrag bg-transparent hover:border border-textarea-background hover:bg-button-hover hover:cursor-pointer    text-sidebar-foreground/50 hover:text-sidebar-foreground transition-colors outline-none"
        >
          <Ellipsis size={16} />
        </Button>
      </DropdownMenuTrigger>
      ...
      >
        ...
        >
          ...
          <span className="text-[10px] text-sidebar-foreground/40">ctrl+d</span>
        </DropdownMenuItem>
        ...
        >
          ...
          <span className="text-[11px] opacity-60">delete / backspace</span>
        </DropdownMenuItem>
      </DropdownMenuContent>
    </DropdownMenu>
  );
