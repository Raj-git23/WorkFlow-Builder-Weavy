...
export function LLMNode(props: NodeProps<LLMRFNode>) {
  ...
  const [imageInputCount, setImageInputCount] = useState(props.data.imageInputCount ?? 0);
  const [comboOpen, setComboOpen] = useState(false);

  const addImageInput = useCallback(() => {
    setImageInputCount((c) => { updateNodeInternals(props.id); return c + 1; });
  ...
  return (
    <BaseNode<LLMNodeData> {...props} handles={handles}>
      {() => (
        <NodeShell title="Run LLM" icon={<span className="text-base">🤖</span>} className="h-96" nodeId={props?.id}>
          ...
          <Popover open={comboOpen} onOpenChange={setComboOpen}>
            ...
            <PopoverContent className="w-full p-0 border-sidebar-border bg-sidebar-background">
              <Command className="bg-transparent text-sidebar-foreground">
                ...
                <CommandInput placeholder="Search models…" className="text-xs text-sidebar-foreground" />
                
                <CommandList>
                  <CommandEmpty className="py-2 text-center text-xs text-sidebar-foreground/80">No model found.</CommandEmpty>
                  <CommandGroup>
                    {MODELS.map((m) => (
                      <CommandItem key={m.value} value={m.value}
                        onSelect={(val) => { setModel(val); setComboOpen(false); }}
                        className="text-xs font-medium text-sidebar-foreground hover:bg-gray-700 cursor-pointer">
                        {m.label}
                        <Check size={12} className={cn("ml-auto", model === m.value ? "opacity-100" : "opacity-0")} />
                      </CommandItem>
                    ))}
                  </CommandGroup>
                </CommandList>
          ...
          <PreviewArea>
            ...
            {promptText && imageInputCount === 0 && (
              <div className="absolute inset-0 p-3 overflow-auto">
                <p className="text-[11px] text-sidebar-foreground/70 leading-relaxed">{promptText}</p>
              </div>
            )}
          </PreviewArea>
          ...
          <div className="flex items-center justify-between mt-1">
            ...
            <Button variant="ghost" size="sm" onClick={addImageInput}
